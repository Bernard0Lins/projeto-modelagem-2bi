...
[cite_start]Este repositório contém o projeto avaliativo do 2º Bimestre da disciplina de **Modelagem Estatística**, ministrada pelo Prof. Pedro Girotto.

[cite_start]O objetivo principal é aplicar técnicas de Ciência de Dados para analisar um dataset de seguros de saúde, realizando limpeza de dados, análise exploratória (EDA), testes de hipótese e construção de modelos preditivos (Regressão e Classificação).
...
* [cite_start]**Licença:** Database Contents License (DbCL) v1.0 / Domínio Público.
...
1.  **Análise Exploratória (EDA):**
    ...
    * [cite_start]Testes de Hipótese (Teste T) para validar impacto do tabagismo nos custos.
...
3.  **Modelagem (Regressão):**
    ...
    * [cite_start]Avaliação: RMSE, MAE, R² e Diagnóstico de Resíduos.
4.  **Modelagem (Classificação):**
    * Objetivo: Prever a variável `smoker` (Detecção de risco).
    * [cite_start]Modelos: Regressão Logística e Naive Bayes (Gaussian).
    * [cite_start]Avaliação: Curva ROC, AUC e Matriz de Confusão.
5.  **Otimização:**
    * [cite_start]Tuning de hiperparâmetros utilizando `GridSearchCV` para o modelo Random Forest.
...
1.  Clone o repositório:
    ```bash
    git clone [https://github.com/SEU_USUARIO/modelagem-estatistica-insurance.git](https://github.com/SEU_USUARIO/projeto-modelagem-2bi.git)
    ```
2.  Entre na pasta do projeto:
    ```bash
    cd modelagem-estatistica-insurance
    ```
3.  [cite_start]Instale as dependências listadas no `requirements.txt`:
    ```bash
...
├── 📄 main.ipynb   # O código completo, análises e relatórios
